...
#ifndef __GNUC__
#error Must be compiled with GNU C (I think)
#endif /* !__GNUC__ */
...
int main(int argc, char *argv[])
{
    ...
    *buf = '\0';
    ...
    /* LJH - added b option */
    while ((c = getopt(argc, argv, "abe:fhvn:o:")) != EOF) {
        switch (c) {
            case 'a':
                options.attributesOnly = 1;
                break;
	    case 'b': /* added this option */
		options.bytecount = 1;
		break;
            case 'e':
...
static void print_usage(void)
{
    fprintf(stderr, "Last compiled: "__DATE__"\n");
...
static void print_help(void)
{
    ...
    /* LJH - addes following line */
    fprintf(stderr, "        -b              Add information about bytecode size.\n");
    fprintf(stderr, "                          (only valid for jasmin output)\n");
