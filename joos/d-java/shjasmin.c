...
int showClassFile_jasmin(ClassFile *cf)
{
	...
	if (options.bytecount)
	  printf("\n; TOTAL NUMBER OF BYTES IN METHODS: %d \n",totalbytes);
    }
...
{   /* LJH added following line */
    if (options.bytecount) printf("; BYTECODE LENGTH: %d \n",c->code_length);
    totalbytes += c->code_length;
...
{
    u1 *code = c->code;
    u4 i, j, align, label_num, lasti;
    short opcode;
    s4 low, high, default_offset, npairs, match, offset;

    /* LJH added +1 in allocation */
    if ((num_labels = scanForLabels(cf, c))) {
        if ((labels = (u4 *)malloc((num_labels + 1) * sizeof(u4))) == NULL) {
            return ERR_MEM;
        }
        /* printf("number of labels after allocation %d \n",num_labels); */
        num_labels = insertLabels(cf, c, labels);
        /* printf("number of labels after insertion %d \n",num_labels); */
    }
    else
      labels = NULL; /* LJH, added - no labels */ 
	...
	switch (opcode) {
	    ...
	    case INVOKEVIRTUAL: case INVOKENONVIRTUAL:
	    case INVOKESTATIC:
    ...
    /* LJH - added next three lines */
    i++;  /* increment for opcode */
    if (options.bytecount) printf(" ; # bytes: %d", i - lasti);
    lasti = i;
    }
    ...
    if (labels != NULL) free(labels); /* LJH added check for null */
