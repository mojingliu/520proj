...
%token tABSTRACT tBOOLEAN tBREAK tBYTE tCASE tCATCH tCHAR tCLASS tCONST
       ...
       tDEC tPLUSEQ tMINUSEQ tANDEQ tDIVEQ tMODEQ tOREQ tTIMESEQ
...
%type <statement> ifthenelsestatementnoshortif whilestatementnoshortif forstatementnoshortif
...
forstatement : tFOR '(' statementexpression ';' expression ';' statementexpression ')' statement
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile($5,makeSTATEMENTsequence($9,makeSTATEMENTexp($7))));
                }
             | tFOR '(' statementexpression ';' expression ';' ')' statement
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile($5,$8));
                }
             | tFOR '(' statementexpression ';' ';' statementexpression ')' statement
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile(makeEXPboolconst(1),makeSTATEMENTsequence($8,makeSTATEMENTexp($6))));
                }
             | tFOR '(' statementexpression ';' ';' ')' statement
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile(makeEXPboolconst(1),$7));
                }
             | tFOR '(' ';' expression ';' statementexpression ')' statement
                {$$ = makeSTATEMENTwhile($4,makeSTATEMENTsequence($8,makeSTATEMENTexp($6)));}
             | tFOR '(' ';' expression ';' ')' statement
                {$$ = makeSTATEMENTwhile($4,$7);}
             | tFOR '(' ';' ';' statementexpression ')' statement
                {$$ = makeSTATEMENTwhile(makeEXPboolconst(1),makeSTATEMENTsequence($7,makeSTATEMENTexp($5)));
                }
             | tFOR '(' ';' ';' ')' statement
                {$$ = makeSTATEMENTwhile(makeEXPboolconst(1), $6);
                }
;

forstatementnoshortif : tFOR '(' statementexpression ';' expression ';' statementexpression ')' statementnoshortif
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile($5,makeSTATEMENTsequence($9,makeSTATEMENTexp($7))));
                }
             | tFOR '(' statementexpression ';' expression ';' ')' statementnoshortif
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile($5,$8));
                }
             | tFOR '(' statementexpression ';' ';' statementexpression ')' statementnoshortif
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile(makeEXPboolconst(1),makeSTATEMENTsequence($8,makeSTATEMENTexp($6))));
                }
             | tFOR '(' statementexpression ';' ';' ')' statementnoshortif
                {$$ = makeSTATEMENTsequence(makeSTATEMENTexp($3),
                makeSTATEMENTwhile(makeEXPboolconst(1),$7));
                }
             | tFOR '(' ';' expression ';' statementexpression ')' statementnoshortif
                {$$ = makeSTATEMENTwhile($4,makeSTATEMENTsequence($8,makeSTATEMENTexp($6)));}
             | tFOR '(' ';' expression ';' ')' statementnoshortif
                {$$ = makeSTATEMENTwhile($4,$7);}
             | tFOR '(' ';' ';' statementexpression ')' statementnoshortif
                {$$ = makeSTATEMENTwhile(makeEXPboolconst(1),makeSTATEMENTsequence($7,makeSTATEMENTexp($5)));
                }
             | tFOR '(' ';' ';' ')' statementnoshortif
                {$$ = makeSTATEMENTwhile(makeEXPboolconst(1), $6);
                }
;
...
assignment : tIDENTIFIER '=' expression
           ...
           | tDEC tIDENTIFIER
             {$$ = makeEXPassign($2, makeEXPminus(makeEXPid($2), makeEXPintconst(1)));}
           | tIDENTIFIER tPLUSEQ expression
             {$$ = makeEXPassign($1, makeEXPplus(makeEXPid($1), $3));}
           | tIDENTIFIER tMINUSEQ expression
             {$$ = makeEXPassign($1, makeEXPminus(makeEXPid($1), $3));}
           | tIDENTIFIER tDIVEQ expression
             {$$ = makeEXPassign($1, makeEXPdiv(makeEXPid($1), $3));}
           | tIDENTIFIER tTIMESEQ expression
             {$$ = makeEXPassign($1, makeEXPtimes(makeEXPid($1), $3));}
           | tIDENTIFIER tMODEQ expression
             {$$ = makeEXPassign($1, makeEXPmod(makeEXPid($1), $3));}
           | tIDENTIFIER tANDEQ expression
             {$$ = makeEXPassign($1, makeEXPand(makeEXPid($1), $3));}
           | tIDENTIFIER tOREQ expression
             {$$ = makeEXPassign($1, makeEXPor(makeEXPid($1), $3));}
;
