...
def show(html, num, **kwargs):
    ...
    counter = num
    if sid == '':
        sid = str('%030x' % random.randrange(256**15))
    save_context()
    print ('<html><form method="POST" action="?' + str(session) + '$' + str(sid) + '">')
    html(**kwargs)
    print ('<input type="submit" value="go" /></form></body></body></html>')
...
def html_output(**kwargs):
    print ("""<head><style>
        ...
        </script></head><body>""" + str(kwargs["p_board"]) + """
        <span>""" + str(kwargs["p_whose"]) + """'s turn</span><br>
        <span style=\"color:red;\">""" + str(kwargs["p_error"]) + """</span>
...
def html_upgrade(**kwargs):
    print ("""<head><style>
        ...
        </style></head><body>""" + str(kwargs["p_board"]) + """
...
def html_hexit(**kwargs):
    print ("""<body>But now it's dead.</body>""")

def fn_getRow(p_rank):
    function_context({"rank": p_rank})
...
def fn_getPiece(p_rank, p_file):
    function_context({"rank": p_rank, "file": p_file})
    if v["file"] == 1:
        toR = (v["rank"]["a"])
        pop_function()
        return toR
...
def fn_setPiece(p_rank, p_file, p_piece):
    function_context({"rank": p_rank, "file": p_file, "piece": p_piece})
    if v["rank"] == 1:
        if v["file"] == 1:
            v["row1"]["a"] = v["piece"]
        elif v["file"] == 2:
...
def fn_movePiece(p_from_rank, p_from_file, p_to_rank, p_to_file):
    function_context({"from_rank": p_from_rank, "from_file": p_from_file, "to_rank": p_to_rank, "to_file": p_to_file})
...
def fn_getBoard(p_pickable):
    function_context({"pickable": p_pickable})
    ...
    if v["pickable"]:
        ...
        v["i"] = 1
        while v["i"] <= 8:
            v["s"] = v["s"] + "<td class=from><input onClick='set_from(" + str(v["i"]) + ", 0);' type='radio' name='file_from' value='" + str(v["i"]) + "'></td>"
            v["i"] = v["i"] + 1
        v["s"] = v["s"] + "<td> </td></tr>"
    v["i"] = 1
    while v["i"] <= 8:
        ...
        v["j"] = 1
        v["s"] = v["s"] + "<tr align=center><td class=from><input onClick='set_from(0, " + str(v["i"]) + ");'  type='radio' name='rank_from' value='" + str(v["i"]) + "'></td>"
        while v["j"] <= 8:
            v["piece"] = fn_getPiece(v["lookup"], v["j"])
            v["s"] = v["s"] + "<td id='r" + str(v["i"]) + "f" + str(v["j"]) + "'>"
            if v["piece"] != 0:
                v["s"] = v["s"] + "<img src=chess_ts/p" + str(v["piece"]) + ".png>"
            v["s"] = v["s"] + "</td>"
            v["j"] = v["j"] + 1
        if v["pickable"]:
            v["s"] = v["s"] + "<td class=to><input onClick='set_to(0, " + str(v["i"]) + ");' type='radio' name='rank_to' value='" + str(v["i"]) + "'></td>"
        v["s"] = v["s"] + "</tr>"
        v["i"] = v["i"] + 1
    if v["pickable"]:
        v["i"] = 1
        v["s"] = v["s"] + "<tr><td> </td>"
        while v["i"] <= 8:
            v["s"] = v["s"] + "<td class=to><input onClick='set_to(" + str(v["i"]) + ", 0);' type='radio' name='file_to' value='" + str(v["i"]) + "'></td>"
            v["i"] = v["i"] + 1
        ...
        while (1 <= counter <= 2) or True:
            if counter <= 0:
                ...
                if v["turn"]:
                    ...
                    v["turn_words"] = "Black"
                return show(html_output, 1, p_error=v["error"], p_whose=v["turn_words"], p_board=v["board"])
            if counter == 1:
                counter = 0
                v["from_rank"] = int(receives.get("rank_from", '0'))
                v["from_file"] = int(receives.get("file_from", '0'))
                v["to_rank"] = int(receives.get("rank_to", '0'))
                v["to_file"] = int(receives.get("file_to", '0'))
            if counter <= 1:
                v["error"] = fn_movePiece(v["from_rank"], v["from_file"], v["to_rank"], v["to_file"])
                
            if counter <= 2:
                if (2 <= counter <= 2) or v["error"] == "!":
                    if counter <= 1:
                        v["board"] = fn_getBoard(False)
                        return show(html_upgrade, 2, p_board=v["board"])
                    if counter == 2:
                        counter = 0
                        v["upgr"] = int(receives.get("upg", '0'))
                    if counter <= 2:
                        if not v["turn"]:
                            v["upgr"] = v["upgr"] + 6
                        fn_setPiece(v["to_rank"], v["to_file"], v["upgr"])
        return exit(html_hexit, 3)
        pop_context()
...
if __name__ == '__main__':
    ...
    if '$' in s:
        i = s.index('$')
        session, sep, sid = s[0:i], s[i], s[i+1:]
    else:
        session, sep, sid = s, '', ''
    if session not in sessions:
        print '<html>You did not enter a valid session name.<br>Try one of these:<br>'
        for s in sessions:
        ...
        try:
            ...
            print "Your session key is not valid.<br>Try one of the sessions below for a new key<br>"
            for s in sessions:
                print " -<a href='?" + s + "'>", s, "</a><br>"
            print "</html>"
        else:
            for i in storage:
                receives[i] = str(storage[i].value)
            sessions[session]()
    else:
        sessions[session]()
